...
/**
 * Alta y corrección de una labor, con sus tareas SAP y sus códigos de
 * implemento.
 *
 * Vive aparte de la pantalla de Vinculación porque se abre desde DOS
 * sitios: esa pantalla y el formulario de labor del ticket, cuando el
 * digitador no encuentra la labor que hizo. Antes estaba dentro de
 * `VinculacionLabores` y hacía `router.refresh()` al guardar; llamada
 * desde el ticket eso recargaba la ruta y le borraba al usuario todo lo
 * que ya llevaba escrito.
 *
 * Por eso no navega: avisa con `onGuardado(laborId)` y quien la abrió
 * decide qué hacer —refrescar la pantalla, o meter la labor nueva en su
 * lista y seleccionarla sin perder nada—.
 *
 * Una labor sin tareas SAP no sirve para capturar: la pantalla de
 * captura no ofrecería ninguna tarea. De ahí que las tareas se elijan
 * aquí mismo y no en un segundo paso.
 */
...
import type { ImplementoFisicoOpcion, LaborVinculada } from '@/lib/labores/tipos'

export { LABOR_NUEVA } from '@/lib/labores/tipos'
...
export function LaborModal({
  ...
  onCerrar: () => void
  /** El id de la labor guardada. Quien abrió el modal decide qué hacer. */
  onGuardado: (laborId: string) => void
}) {
